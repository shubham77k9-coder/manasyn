...
async function sendMessage() {
  ...
  try {
    const res = await fetch('/api/chat', {
      ...
      body: JSON.stringify({ conversation_id: parseInt(convId), message }),
    });
    ...
    if (data.success) {
      ...
      appendMessage('assistant', 'Something interrupted the conversation. Your message wasn\'t lost. Please try again.');
    }
  } catch (err) {
    removeTypingIndicator();
    appendMessage('assistant', 'Something interrupted the conversation. Your message wasn\'t lost. Please try again.');
