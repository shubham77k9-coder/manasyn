...
router.get('/dashboard', requireAuth, (req, res) => {
  ...
  const weekActivity = db.prepare(`
    SELECT DATE(created_at) as day, COUNT(*) as count FROM (
      ...
      UNION ALL SELECT completed_at FROM self_work_exercises WHERE user_id = ? AND completed_at >= date('now', '-7 days')
...
router.get('/talk/:id', requireAuth, (req, res) => {
  ...
  if (!conv) { setFlash(req, 'error', 'Conversation not found.'); return res.redirect('/app/talk'); }
...
router.post('/reflect', requireAuth, (req, res) => {
  ...
  db.prepare('INSERT INTO reflections (user_id, emotion, trigger, pattern, insight) VALUES (?, ?, ?, ?, ?)').run(userId, emotion || '', trigger || '', pattern || '', insight || '');
...
router.post('/self-work/:id/complete', requireAuth, (req, res) => {
  const userId = req.session.user.id;
  db.prepare("UPDATE self_work_exercises SET status = 'completed', completed_at = datetime('now') WHERE id = ? AND user_id = ?").run(req.params.id, userId);
...
router.post('/journal', requireAuth, (req, res) => {
  ...
  if (!content) { setFlash(req, 'error', 'Please write something before saving.'); return res.redirect('/app/journal'); }
  db.prepare('INSERT INTO journal_entries (user_id, title, content, mood) VALUES (?, ?, ?, ?)').run(userId, title || '', content, mood || '');
...
router.post('/settings/preferences', requireAuth, (req, res) => {
  ...
  db.prepare('UPDATE user_settings SET theme = ?, notifications_enabled = ?, voice_responses = ?, reduced_motion = ?, language = ? WHERE user_id = ?')
    .run(theme || 'light', notifications_enabled ? 1 : 0, voice_responses ? 1 : 0, reduced_motion ? 1 : 0, language || 'en', userId);
