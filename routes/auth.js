...
router.get('/login', redirectIfAuth, (req, res) => {
  res.render('auth/login', { title: 'Welcome back — Manasyn' });
});
...
router.post('/signup', (req, res) => {
  ...
  if (step === '4') {
    ...
    const finalRole = role || 'personal';
    const focusGoals = Array.isArray(goals) ? goals.join(', ') : (goals || '');

    const result = db.prepare(`INSERT INTO users (name, email, password, role, onboarding_complete, onboarding_intent, onboarding_focus) VALUES (?, ?, ?, ?, 1, ?, ?)`).run(name || '', email, hash, finalRole, intent || '', focusGoals);

    db.prepare('INSERT OR IGNORE INTO user_settings (user_id) VALUES (?)').run(result.lastInsertRowid);

    req.session.user = { id: result.lastInsertRowid, name: name || '', email, role: finalRole, onboarding_complete: 1 };
...
router.get('/onboarding', requireAuth, (req, res) => {
  if (req.session.user.onboarding_complete) return res.redirect('/app/dashboard');
  res.render('auth/onboarding', { title: 'Let\'s get started — Manasyn' });
});

router.post('/onboarding', requireAuth, (req, res) => {
  const { intent } = req.body;
  db.prepare('UPDATE users SET onboarding_complete = 1, onboarding_intent = ? WHERE id = ?').run(intent || '', req.session.user.id);
...
router.get('/forgot-password', (req, res) => { res.render('auth/forgot-password', { title: 'Reset password — Manasyn' }); });
router.post('/forgot-password', (req, res) => { setFlash(req, 'info', 'If an account exists for that email, a reset link has been sent.'); res.redirect('/auth/login'); });

// ── Helper: seed default self-work ──
