...
router.get('/dashboard', requireStudent, (req, res) => {
  ...
  res.render('student/dashboard', {
    title: 'Student Dashboard — Manasyn',
    progress, cases, osceQs, flashcards,
  });
});
...
router.get('/case-coach/:id', requireStudent, (req, res) => {
  ...
  if (!caseData) {
    setFlash(req, 'error', 'Case not found.');
    return res.redirect('/student/case-coach');
  }
...
router.post('/case-coach/:id/respond', requireStudent, (req, res) => {
  ...
  db.prepare('INSERT INTO case_responses (user_id, case_id, action_type, response_text, feedback) VALUES (?, ?, ?, ?, ?)')
    .run(userId, req.params.id, action_type || '', response_text || '', feedback);
...
router.get('/osce/:id', requireStudent, (req, res) => {
  ...
  if (!question) {
    setFlash(req, 'error', 'Question not found.');
    return res.redirect('/student/osce');
  }
...
router.post('/osce/:id/respond', requireStudent, (req, res) => {
  ...
  db.prepare('INSERT INTO osce_responses (user_id, question_id, user_answer, clinical_reasoning, communication, ethics, structure, feedback) VALUES (?, ?, ?, ?, ?, ?, ?, ?)')
    .run(userId, req.params.id, user_answer || '', scores.clinical_reasoning, scores.communication, scores.ethics, scores.structure, scores.feedback);
...
router.get('/skills-lab/:id', requireStudent, (req, res) => {
  ...
  if (!scenario) {
    setFlash(req, 'error', 'Scenario not found.');
    return res.redirect('/student/skills-lab');
  }
...
router.post('/skills-lab/:id/respond', requireStudent, (req, res) => {
  ...
  db.prepare('INSERT INTO skills_responses (user_id, scenario_id, chosen_option, is_correct) VALUES (?, ?, ?, ?)')
    .run(userId, req.params.id, chosen_option || '', isCorrect);
...
router.get('/flashcards/deck/:id', requireStudent, (req, res) => {
  ...
  if (!deck) {
    setFlash(req, 'error', 'Deck not found.');
    return res.redirect('/student/flashcards');
  }
...
router.post('/flashcards/:id/review', requireStudent, (req, res) => {
  ...
  db.prepare('INSERT INTO flashcard_reviews (user_id, flashcard_id, rating) VALUES (?, ?, ?)')
    .run(userId, req.params.id, rating || 'good');
  res.json({ success: true });
});

// ── Research Helper ──
router.get('/research', requireStudent, (req, res) => {
  res.render('student/research', { title: 'Research Helper — Manasyn' });
});
...
function generateOsceScores(answer) {
  if (!answer || answer.length < 20) {
    return {
      clinical_reasoning: 4, communication: 5, ethics: 4, structure: 4,
      feedback: 'Your answer was brief. In a viva, elaborate on your clinical reasoning and demonstrate structured thinking.',
    };
  }
